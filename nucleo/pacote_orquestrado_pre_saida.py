...
PENDENTE_AMBIGUO = "pendente_ambigua_v17_c1"
PENDENTE_ESTADO = "pendente_estado_temporal_futuro_v17_c1"
PENDENTE_AUSENTE = "pendente_fonte_ausente_v17_c1"
...
def _get(row: pd.Series, col: str, padrao: Any = "") -> Any:
...
def _montar_recomendacoes_futuras(contexto: Any) -> pd.DataFrame:
    ...
    for i, row in df.iterrows():
        linhas.append({
            ...
            "conta_descricao": _valor(row, df, ["conta", "Conta", "descricao", "Descrição", "descrição"], ""),
            "valor_pagamento": _valor(row, df, ["valor_pagamento", "Valor", "valor"], ""),
            "status_recomendacao": _valor(row, df, ["status_recomendacao", "Status recomendação", "Status recomendacao", "status"], ""),
            "motivo_bloqueio": _valor(row, df, ["motivo_bloqueio", "Motivo bloqueio lote", "motivo"], ""),
            "pacote_do_dia": _valor(row, df, ["pacote_do_dia", "Pacote do dia", "Estratégia", "estrategia"], ""),
            "necessita_switching": _valor(row, df, ["necessita_switching", "Necessita switching"], ""),
            "fonte_verdade_origem": PENDENTE_AMBIGUO,
        })
...
def _montar_decisoes_pagamento(contexto: Any) -> pd.DataFrame:
    ...
    linhas = []
    for i, row in df.iterrows():
        tipo_antigo = _norm(_valor(row, df, ["tipo_fonte_escolhida", "tipo_fonte", "tipo_fonte_candidata"], ""))
        tipo_v17 = TIPOS_V17_POR_FONTE_ANTIGA.get(tipo_antigo, tipo_antigo or PENDENTE_AMBIGUO)
        linhas.append({
            "pagamento_id": _valor(row, df, ["pagamento_id", "despesa_id", "Despesa ID", "id"], f"pagamento_auto_{i}"),
            "tipo_fonte_escolhida_v17": tipo_v17,
            "fonte_id": _valor(row, df, ["fonte_id", "lote_id", "lote_escolhido", "Lote sugerido", "lote"], ""),
            "valor_bruto_resgate": _valor(row, df, ["valor_bruto_resgate", "Bruto"], PENDENTE_AMBIGUO),
            "valor_imposto": _valor(row, df, ["valor_imposto", "Imposto", "IR"], PENDENTE_AMBIGUO),
            "valor_liquido_resgate": _valor(row, df, ["valor_liquido_resgate", "Líquido", "Liquido"], PENDENTE_AMBIGUO),
            "saldo_antes": PENDENTE_ESTADO,
            "saldo_depois": PENDENTE_ESTADO,
            "status_decisao": _valor(row, df, ["status_decisao", "status", "Status recomendação", "Status recomendacao"], ""),
        })
    return pd.DataFrame(linhas)
...
def _montar_estado_temporal_switching(contexto: Any) -> pd.DataFrame:
    ...
    for i, row in df.iterrows():
        ...
        linhas.append({
            ...
            "status_reconciliacao": "materializavel_diagnostico_v17_c1" if str(destino or "").strip() else PENDENTE_AUSENTE,
        })
    return pd.DataFrame(linhas)
...
def _auditoria_componentes(componentes: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for nome, df in componentes.items():
        ...
        linhas.append({
            "componente": nome,
            "origem_funcional": "montador_v17_c1",
            ...
            "mensagem_auditoria": "V17-C1 preenche campos seguros e preserva pendencias sem inventar valor",
        })
    return pd.DataFrame(linhas)
