...
def _mapa_rank_por_produto(candidatos: pd.DataFrame) -> dict[str, int]:
    ...
    return {limpar_texto(row.get('produto_key')): int(idx + 1) for idx, (_, row) in enumerate(ordenado.iterrows())}
...
def carregar_switching_economico_shadow(
    ...
    *,
    ...
    rank_por_produto = _mapa_rank_por_produto(candidatos)
    ...
    for lote in lotes:
        ...
        for _, produto_row in candidatos.iterrows():
            ...
            linha = {
                ...
                'rank_origem': int(rank_por_produto.get(limpar_texto(lote.produto_key), 999)),
    ...
    if len(elegiveis) > 0:
        ...
        elegiveis.loc[gate_top1_risco, 'motivo_gate_switching'] = 'bloqueado_origem_top1_risco_liquidez'
        elegiveis.loc[gate_top1_risco, 'recomendado_shadow'] = False
