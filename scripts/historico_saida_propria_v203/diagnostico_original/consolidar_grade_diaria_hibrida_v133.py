...
RELATORIO = Path(RAIZ) / 'relatorios' / 'atuais' / 'GRADE_DIARIA_OFICIAL_HIBRIDA_V133.md'
JSON_OUT = Path(RAIZ) / 'saidas' / 'operacional' / 'grade_diaria_oficial_hibrida_v133_consolidado.json'
...
def consolidar() -> dict:
    chunks = sorted(BASE_OPERACIONAL.glob('grade_diaria_hibrida_v133_offset_*.json'))
    if not chunks:
        raise FileNotFoundError('Nenhum chunk V133 encontrado em saidas/operacional.')
    resultados = []
    dias = []
    for chunk in chunks:
