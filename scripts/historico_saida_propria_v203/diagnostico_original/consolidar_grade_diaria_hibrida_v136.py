...
RELATORIO = Path(RAIZ) / 'relatorios' / 'atuais' / 'GRADE_DIARIA_OFICIAL_HIBRIDA_V136.md'
JSON_OUT = Path(RAIZ) / 'saidas' / 'operacional' / 'grade_diaria_oficial_hibrida_v136_consolidado.json'
...
JANELA_INICIO = _date_env('V136_JANELA_INICIO', '2026-05-21')
JANELA_FIM = _date_env('V136_JANELA_FIM', '2027-03-31')
...
def consolidar() -> dict:
    chunks = sorted(BASE_OPERACIONAL.glob(f'grade_diaria_hibrida_v136_{JANELA_INICIO}_{JANELA_FIM}_offset_*.json'))
    if not chunks:
        raise FileNotFoundError('Nenhum chunk V136 encontrado em saidas/operacional.')
    resultados = []
    dias = []
    for chunk in chunks:
    ...
    linhas = [
        '# Grade diária oficial com comparador híbrido — V136',
        '',
