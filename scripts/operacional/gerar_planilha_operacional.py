...
def main() -> None:
    ...
    for item in gastos_futuros.to_dict('records'):
        ...
        rows_futuro.append([
            ...
            round(float(central.get('penalidade_escassez_protegida_futura_bruta') or 0.0), 4) if central and central.get('penalidade_escassez_protegida_futura_bruta') is not None else '',
            round(float(central.get('cap_penalidade_escassez_protegida_futura') or 0.0), 4) if central and central.get('cap_penalidade_escassez_protegida_futura') is not None else '',
            round(float(central.get('penalidade_deficit_atual') or 0.0), 6) if central and central.get('penalidade_deficit_atual') is not None else '',
            'sim' if bool(central.get('fonte_preservada_por_reserva')) else '',
            central.get('fonte_preservada_referencia', '') if central else '',
            'sim' if bool(central.get('fallback_sem_fonte_viavel')) else '',
            central.get('motivo_indisponibilidade_central', '') if central else '',
    ...
    if quadro_central_full is not None and len(quadro_central_full):
        ...
        for _, row_c in quadro_central_full.iterrows():
            rows_central.append([
                ...
                'sim' if bool(row_c.get('fonte_critica_para_protegida_futura')) else '', row_c.get('penalidade_escassez_protegida_futura_bruta'),
                row_c.get('cap_penalidade_escassez_protegida_futura'), row_c.get('penalidade_deficit_atual'), 'sim' if bool(row_c.get('fonte_preservada_por_reserva')) else '',
                row_c.get('fonte_preservada_referencia'), 'sim' if bool(row_c.get('fallback_sem_fonte_viavel')) else '', row_c.get('motivo_indisponibilidade_central'), row_c.get('saldo_antes_central'), row_c.get('bruto_central'),
