...
def main() -> None:
    ...
    if quadro_central_full is not None and len(quadro_central_full):
        ...
        for _, row_c in quadro_central_full.iterrows():
            rows_central.append([
                ...
                row_c.get('demanda_protegida_critica_futura_ponderada'), row_c.get('demanda_protegida_critica_futura_7d'), row_c.get('demanda_protegida_critica_futura_14d'), row_c.get('demanda_protegida_critica_futura_21d'),
                'sim' if bool(row_c.get('fonte_critica_para_protegida_futura')) else '', 'sim' if bool(row_c.get('fonte_critica_para_protegida_critica_futura')) else '', row_c.get('penalidade_escassez_protegida_futura_bruta'), row_c.get('penalidade_escassez_protegida_critica_futura'), row_c.get('penalidade_escassez_protegida_critica_futura_bruta'), 'sim' if bool(row_c.get('perda_relevante_protegida_critica')) else '', row_c.get('orcamento_maximo_perda_protegida'),
                row_c.get('orcamento_perda_protegida_consumido'), row_c.get('orcamento_perda_protegida_restante'), row_c.get('orcamento_perda_protegida_estimado'),
                'sim' if bool(row_c.get('excecao_cartao_material_aplicada')) else '', 'sim' if bool(row_c.get('bloqueio_excecao_antecipacao_protegida')) else '', 'sim' if bool(row_c.get('bloqueio_excecao_orcamento_protegida')) else '', 'sim' if bool(row_c.get('bloqueio_excecao_perda_protegida_critica')) else '',
                'sim' if bool(row_c.get('fonte_preservada_por_reserva')) else '', row_c.get('fonte_preservada_referencia'), 'sim' if bool(row_c.get('fallback_sem_fonte_viavel')) else '', row_c.get('motivo_indisponibilidade_central'), row_c.get('saldo_antes_central'), row_c.get('bruto_central'),
    ...
    headers_central = ['Data', 'Conta', 'Despesa ID', 'Valor', 'Classe operacional', 'Subclasse operacional', 'Prioridade intraclasse', 'Lote local', 'Lote central', 'Tipo fonte central', 'Mudou vs decisão local', 'Status central', 'Score proxy central', 'Violação protegida', 'Severidade protegida', 'Déficit líquido total', 'Patrimônio terminal proxy', 'Penalidade estratégica', 'Penalidade fragmentação', 'Penalidade escassez protegida futura', 'Demanda protegida futura ponderada', 'Demanda protegida 7d', 'Demanda protegida 14d', 'Demanda protegida 21d', 'Demanda protegida crítica ponderada', 'Demanda protegida crítica 7d', 'Demanda protegida crítica 14d', 'Demanda protegida crítica 21d', 'Fonte crítica para protegida futura', 'Fonte crítica para protegida crítica futura', 'Penalidade escassez protegida futura bruta', 'Penalidade escassez protegida crítica', 'Penalidade escassez protegida crítica bruta', 'Perda relevante protegida crítica', 'Orçamento perda protegida máximo', 'Orçamento perda protegida consumido', 'Orçamento perda protegida restante', 'Orçamento perda protegida estimado', 'Exceção cartão material aplicada', 'Bloqueio por trava protegida', 'Bloqueio por orçamento protegido', 'Bloqueio por perda protegida crítica', 'Fonte preservada por reserva', 'Fonte preservada referência', 'Fallback sem fonte viável', 'Motivo indisponibilidade central', 'Saldo Antes central', 'Bruto central', 'Imposto central', 'Líquido central', 'Saldo Remanescente central', 'Cobertura central integral', 'Observação central']
