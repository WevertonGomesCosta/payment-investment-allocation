...
def main() -> None:
    ...
    if quadro_central_full is not None and len(quadro_central_full):
        ...
        for _, row_c in quadro_central_full.iterrows():
            rows_central.append([
                ...
                'sim' if bool(row_c.get('fonte_critica_para_protegida_futura')) else '', row_c.get('penalidade_escassez_protegida_futura_bruta'), row_c.get('orcamento_maximo_perda_protegida'),
                ...
                'sim' if bool(row_c.get('excecao_cartao_material_aplicada')) else '', 'sim' if bool(row_c.get('bloqueio_excecao_antecipacao_protegida')) else '', 'sim' if bool(row_c.get('bloqueio_excecao_orcamento_protegida')) else '',
    ...
    headers_central = ['Data', 'Conta', 'Despesa ID', 'Valor', 'Classe operacional', 'Subclasse operacional', 'Prioridade intraclasse', 'Lote local', 'Lote central', 'Tipo fonte central', 'Mudou vs decisão local', 'Status central', 'Score proxy central', 'Violação protegida', 'Severidade protegida', 'Déficit líquido total', 'Patrimônio terminal proxy', 'Penalidade estratégica', 'Penalidade fragmentação', 'Penalidade escassez protegida futura', 'Demanda protegida futura ponderada', 'Demanda protegida 7d', 'Demanda protegida 14d', 'Demanda protegida 21d', 'Fonte crítica para protegida futura', 'Penalidade escassez protegida futura bruta', 'Orçamento perda protegida máximo', 'Orçamento perda protegida consumido', 'Orçamento perda protegida restante', 'Orçamento perda protegida estimado', 'Exceção cartão material aplicada', 'Bloqueio por trava protegida', 'Bloqueio por orçamento protegido', 'Fonte preservada por reserva', 'Fonte preservada referência', 'Fallback sem fonte viável', 'Motivo indisponibilidade central', 'Saldo Antes central', 'Bruto central', 'Imposto central', 'Líquido central', 'Saldo Remanescente central', 'Cobertura central integral', 'Observação central']
