...
from scripts.diagnostico import inspecionar_grade_diaria_switching_v126 as grade

SAIDA_JSON = Path(RAIZ) / 'saidas' / 'operacional' / 'grade_diaria_switching_v126_consolidado.json'
SAIDA_MD = Path(RAIZ) / 'relatorios' / 'atuais' / 'AVALIACAO_DIARIA_DATA_OTIMA_SWITCHING_V126.md'


def main() -> int:
    base = Path(RAIZ) / 'saidas' / 'operacional'
    chunks = sorted(base.glob('grade_diaria_switching_v126_chunk_*.json'))
    if not chunks:
