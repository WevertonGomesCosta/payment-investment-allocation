...
def main():
    ...
    recebidos = _rows(contexto.recebidos_auditaveis)
    resumo_rec = _rows(saida.resumo_recebidos)
    extrato = _rows(saida.extrato_futuro)
    inventario = _rows(contexto.dados_operacionais.inventario_canonico)
    switchings = _rows(contexto.dados_operacionais.switching_canonico)
    ...
    resumo_mes = _mes_valor_qtd(resumo_rec, ["Data", "data", "Data Recebimento", "data_recebimento", "mes"], ["Valor", "valor", "total", "valor_total", "Valor líquido"])
    ...
    meses = sorted(set(list(sal_bruto.keys()) + list(sal_liq.keys()) + list(rec_mes.keys()) + list(resumo_mes.keys()) + list(pag_mes.keys()) + list(aportes_mes.keys()) + list(sw_mes.keys()) + list(pag_sem_cov.keys()) + list(recv_futuro_nao_aportado_mes.keys())))

    rows = []
    for mes in meses:
        salario_bruto = sal_bruto[mes]["total"]
        salario_liquido = sal_liq[mes]["total"]
        qtd_sal = sal_liq[mes]["qtd"] or sal_bruto[mes]["qtd"]
        receb_total = rec_mes[mes]["total"]
        qtd_receb = rec_mes[mes]["qtd"]
        resumo_total = resumo_mes[mes]["total"]
        pag_total = pag_mes[mes]["total"]
        qtd_pag = pag_mes[mes]["qtd"]
        qtd_sem_cov = pag_sem_cov[mes]["qtd"]
        val_sem_cov = pag_sem_cov[mes]["total"]
        aporte_total = aportes_mes[mes]["total"]
        qtd_aporte = aportes_mes[mes]["qtd"]
        sw_total = sw_mes[mes]["total"]
        qtd_sw = sw_mes[mes]["qtd"]
        saldo_est = receb_total - aporte_total - pag_total
        d_sal_rec = salario_liquido - receb_total
        d_rec_res = receb_total - resumo_total
    ...
    meses_div_sal_rec = int((df["diferenca_salarios_vs_recebidos"].abs() > 0.01).sum()) if not df.empty else 0
    ...
    print(f"total_reconciliado_recebidos={df['recebidos_auditaveis_total'].sum() if not df.empty else 0.0:.2f}")
