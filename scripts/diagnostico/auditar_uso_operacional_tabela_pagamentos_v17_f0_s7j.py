...
COLS = [
    ...
    'status_operacional','acao_recomendada','saldo_pos_pagamento','saldo_pos_pagamento_origem','alerta_operacional','tipo_alerta_operacional',
    'problema_operacional','motivo_operacional','fonte_aprovada_para_pagamento'
]
...
def main():
    fonte='indisponivel'; caminho='indisponivel'; df=None
    if XLSX.exists():
        xls=pd.ExcelFile(XLSX)
        if ABA in xls.sheet_names:
            df=pd.read_excel(XLSX, sheet_name=ABA)
            fonte='xlsx'; caminho=str(XLSX)
    if df is None and CSV_S7G.exists():
        df=pd.read_csv(CSV_S7G)
        fonte='csv_s7g'; caminho=str(CSV_S7G)
    ...
    aprov['_data']=pd.to_datetime(s(aprov,'data'),errors='coerce'); aprov=aprov.sort_values(['_data','conta'])
    cols_show=['data','conta','valor','lote_recomendado','fonte_principal','fonte_reserva','qtd_fontes_componentes','status_operacional','acao_recomendada','saldo_pos_pagamento','saldo_pos_pagamento_origem','usa_lote_pos_switching','qtd_componentes_pos_switching']
    cols_show=[c for c in cols_show if c in aprov.columns]
    print('\nproximos_5_pagamentos_aprovados=')
    print(aprov[cols_show].head(5).to_string(index=False))
    print('\nproximos_10_pagamentos_aprovados=')
    print(aprov[cols_show].head(10).to_string(index=False))
    ate=aprov[aprov['_data']<=pd.Timestamp('2026-06-30')]
    print('\npagamentos_aprovados_ate_2026_06_30=')
    print(ate[cols_show].to_string(index=False) if not ate.empty else 'nenhum')
    ...
    print('\namostra_multifonte_10=')
    print(multi[mcols].head(10).to_string(index=False))

    aexp=df[alerta_exp_mask]
    acols=[c for c in ['data','conta','valor','status_operacional','acao_recomendada','problema_operacional','motivo_operacional','saldo_pos_pagamento','saldo_pos_pagamento_origem'] if c in aexp.columns]
    print('\namostra_alertas_explicitos_10=')
    print(aexp[acols].head(10).to_string(index=False))

    sem=df[sem_lote_mask]
    scols=[c for c in ['data','conta','valor','status_operacional','acao_recomendada','alerta_operacional','tipo_alerta_operacional','problema_operacional','motivo_operacional','saldo_pos_pagamento','saldo_pos_pagamento_origem'] if c in sem.columns]
    print('\namostra_sem_lote_10=')
    print(sem[scols].head(10).to_string(index=False))

    uso=aprov.copy()
    uso['usar_lote']=s(uso,'lote_recomendado')
    uso['usar_fonte_principal']=s(uso,'fonte_principal')
    uso['usar_fonte_reserva']=s(uso,'fonte_reserva')
    uso['tipo_pagamento']=s(uso,'status_operacional')
    uso['observacao_operacional']=''
    ...
    ucols=['data','conta','valor','usar_lote','usar_fonte_principal','usar_fonte_reserva','tipo_pagamento','saldo_pos_pagamento','observacao_operacional']
    ucols=[c for c in ucols if c in uso.columns]
    print('\nuso_operacional_resumo=')
    print(uso[ucols].head(20).to_string(index=False))

    print(f'sentinela_uso_internet_ok={sent_uso(df,"2026-05-15","Internet","Lote 3120 mai",2895.01)}')
    print(f'sentinela_uso_cartao_azul_ok={sent_uso(df,"2026-05-20","Cartão Azul","Lote 3120 mai + Lote 3000 mai Neon",869.53)}')
    print(f'sentinela_uso_condominio_2026_05_20_ok={sent_uso(df,"2026-05-20","Condomínio","Lote 3000 mai Neon",1205.69)}')
    print(f'sentinela_uso_implante_velt_ok={sent_uso(df,"2026-05-30","Implante Velt","Lote 3120 mai",2495.01)}')
    print(f'sentinela_uso_cartao_nu_ok={sent_uso(df,"2026-06-02","Cartão NU","Lote 3120 mai",1915.01)}')
    print(f'sentinela_alerta_aluguel_ok={sent_alerta(df,"2026-06-12","Aluguel")}')
    print(f'sentinela_alerta_condominio_2026_06_20_ok={sent_alerta(df,"2026-06-20","Condomínio")}')

    try:
        CSV_S7J.parent.mkdir(parents=True, exist_ok=True)
        save_cols=['data','conta','valor','usar_lote','usar_fonte_principal','usar_fonte_reserva','tipo_pagamento','status_operacional','acao_recomendada','alerta_operacional','tipo_alerta_operacional','problema_operacional','motivo_operacional','saldo_pos_pagamento','observacao_operacional']
        save_cols=[c for c in save_cols if c in uso.columns]
        uso[save_cols].to_csv(CSV_S7J, index=False)
        csv_out=str(CSV_S7J)
    except Exception:
        csv_out='nao_gerado'
    print(f'csv_s7j={csv_out}')

    ok=(len(df)==159 and len(falt)==0 and data_ok==159 and conta_ok==159 and valor_ok==159 and status_ok==159 and acao_ok==159 and aprovado_pay==33 and aprovado_multi==16 and aprov_total==49 and qtd_multi==16 and comp_multi==32 and sem_lote==110 and sem_lote_alerta==110 and sem_lote_sem_alerta==0 and alerta_exp==110 and alerta_inf==0 and lote_pos==14 and comp_pos==16 and sent_uso(df,"2026-05-15","Internet","Lote 3120 mai",2895.01)=='sim' and sent_uso(df,"2026-05-20","Cartão Azul","Lote 3120 mai + Lote 3000 mai Neon",869.53)=='sim' and sent_uso(df,"2026-05-20","Condomínio","Lote 3000 mai Neon",1205.69)=='sim' and sent_uso(df,"2026-05-30","Implante Velt","Lote 3120 mai",2495.01)=='sim' and sent_uso(df,"2026-06-02","Cartão NU","Lote 3120 mai",1915.01)=='sim' and sent_alerta(df,"2026-06-12","Aluguel")=='sim' and sent_alerta(df,"2026-06-20","Condomínio")=='sim')
