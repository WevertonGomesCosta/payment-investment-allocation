...
def main():
    ...
    # fonte canonica gastos_canonicos
    fonte_base_linhas='nao_determinado'
    nome_aba_despesas=''
    colunas_faltantes=[]
    gc=None
    if hasattr(ctx,'dados_operacionais') and hasattr(ctx.dados_operacionais,'gastos_canonicos'):
        gc=ctx.dados_operacionais.gastos_canonicos
        nome_aba_despesas=str(getattr(ctx.dados_operacionais,'nome_aba_despesas','') or '')
    if isinstance(gc,pd.DataFrame) and not gc.empty:
        req=['despesa_id','data','descricao','valor','pago','lote_usado_1','lote_usado_2','passado_pago_ate_data_referencia']
        colunas_faltantes=[c for c in req if c not in gc.columns]
        ...
        if colunas_faltantes:
            ...
            for _,r in cand.iterrows():
                ...
                if is_pos:
                    row=_empty_row(); row.update({'origem_pagamento':'pagamentos_passados_base_canonica','fonte_base_operacional_gastos':fonte_base,'fonte_base_operacional_gastos_status':fonte_status,'fonte_auxiliar_observavel':fonte_aux,'fonte_pagamentos_passados':'saida_canonica','pagamento_id':r.get('despesa_id'),'data_pagamento':r.get('data'),'conta':r.get('descricao'),'valor_pagamento':r.get('valor'),'pagamento_ok_na_planilha':'sim','lote_usado_planilha':lu,'lote_sugerido':lu,'fonte_eh_lote_pos_switching':'sim','fonte_pos_switching':'sim','pos_sw_flag':'sim','presente_no_extrato_futuro':'nao','presente_no_extrato_passado':'nao_determinado','baixa_refletida_situacao_atual':'nao_determinado','divergencia_baixa_pos_switching':'nao_confirmado','tipo_divergencia_q1':'baixa_pos_switching_sem_evidencia_observavel','camada_onde_falha':'saida_observavel','recomendacao_q1':'V17-F0-Q.1.5-B'})
                    detalhes.append(row)
    # casos explicitos
    ...
    resumo={
      ...
      'fonte_base_operacional_gastos_linhas':fonte_base_linhas,'nome_aba_despesas':nome_aba_despesas,'colunas_gastos_canonicos_faltantes':('nenhuma' if not colunas_faltantes else '|'.join(colunas_faltantes)),'qtd_pagamentos_passados_base_canonica': (int(((gc['pago']==True)&(gc['passado_pago_ate_data_referencia']==True)).sum()) if isinstance(gc,pd.DataFrame) and fonte_status=='localizada_canonica' else 'nao_determinado'),'qtd_pagamentos_passados_ok_usando_lote_pos_switching':(int(((pd.DataFrame(detalhes)['origem_pagamento']=='pagamentos_passados_base_canonica')).sum()) if fonte_status=='localizada_canonica' else 'nao_determinado'),'qtd_pagamentos_passados_pos_switching_ausentes_extrato_passado':qaus,'qtd_baixas_passadas_pos_switching_nao_refletidas_situacao_atual':qsit,
      'qtd_casos_explicitos_auditados':2,'qtd_casos_explicitos_encontrados_base_operacional':qenc,'qtd_casos_explicitos_presentes_extrato_passado':'nao_determinado','qtd_casos_explicitos_ausentes_extrato_passado':qce,
      ...
      'qtd_pagamentos_futuros_pos_sem_baixa_aplicavel':int((fut['fonte_eh_lote_pos_switching']=='sim').sum()),'linhas_futuras_pos_marcadas_como_divergencia_confirmada':int(((fut['fonte_eh_lote_pos_switching']=='sim') & (fut['divergencia_baixa_pos_switching']=='sim')).sum()),'qtd_pagamentos_pos_switching_divergencia_confirmada':int((pd.DataFrame(detalhes)['divergencia_baixa_pos_switching'].astype(str)=='sim').sum()),'qtd_pagamentos_pos_switching_divergencia_nao_confirmada':int((pd.DataFrame(detalhes)['divergencia_baixa_pos_switching'].astype(str)=='nao_confirmado').sum()),'qtd_divergencias_baixa_pos_switching':qtd_div,'camada_falha_dominante':camada,'status_geral_q1':status,'q1_alinhado_com_q0':alinh,'q0_status':q0_status,'q0_motivo':q0_motivo,'matching_pos_switching':'token_exato','usa_substring_global_para_matching':'nao','tokens_pos_switching_testados':'sim','dados_financeiros_modificado_apos_execucao':mod
    }
    ...
    print('=== AUDITORIA V17-F0-Q.1.5 — EXPOSICAO TODOS OS GASTOS AO AUDITOR ===')
