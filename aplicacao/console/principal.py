...
def _render_situacao_atual_oficial_minima(
    ...
    _imprimir_pares([
        ...
        ('valor original total', patrimonio_total.get('valor_original', 'nao_materializado')),
        ('patrimônio líquido total', patrimonio_total.get('patrimonio_liquido', 'nao_materializado')),
        ('rendimento líquido total', patrimonio_total.get('rendimento_liquido', 'nao_materializado')),
        ('status valores patrimoniais', patrimonio_total.get('status_auditoria', 'nao_materializado')),
    ])

    colunas_lotes = ['lote_id_operacional', 'status_ciclo', 'data_inicio_rendimento', 'data_fim_rendimento', 'regra_data_fim_rendimento', 'valor_original', 'bruto_sacado', 'liquido_sacado', 'bruto_atual', 'liquido_atual', 'patrimonio_liquido', 'rendimento_liquido', 'status_auditoria']

    print('\n- lotes exauridos:')
    if lotes_exauridos:
        _imprimir_tabela(colunas_lotes, lotes_exauridos, limite=None)
    else:
        print('  lotes_exauridos_nao_materializados_no_pacote_saida_observavel_oficial')

    print('\n- lotes ativos:')
    if lotes_ativos:
        _imprimir_tabela(colunas_lotes, lotes_ativos, limite=None)
    else:
        print('  lotes_ativos_nao_materializados_no_pacote_saida_observavel_oficial')

    print('\n- lotes migrados por switching:')
    if lotes_migrados:
        _imprimir_tabela(colunas_lotes + ['data_switching'], lotes_migrados, limite=None)
    else:
        print('  lotes_migrados_nao_materializados_no_pacote_saida_observavel_oficial')

    print('\n- patrimônio total dos lotes:')
    _imprimir_tabela(
        ['Métrica', 'Valor'],
        [{'Métrica': chave, 'Valor': valor} for chave, valor in patrimonio_total.items()],
        limite=None,
    )
