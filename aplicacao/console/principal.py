...
from aplicacao.console.secoes_financeiras import render_secao_amostras_pagamentos, render_secao_auditoria_temporal_pagamentos, render_secao_metodo_pagamentos, render_secao_nucleo, render_secao_recomputacao_sequencial_pagamentos, render_secao_reescolha_dinamica_pagamentos, render_secao_replay, render_secao_situacao_atual
...
def main() -> None:
    ...
    render_secao_recomputacao_sequencial_pagamentos(
        auditoria_recomputacao=contexto_baseline.recomputacao_sequencial_preventiva.auditoria if contexto_baseline.recomputacao_sequencial_preventiva is not None else {},
        amostra_trocas_preventivas=((contexto_baseline.recomputacao_sequencial_preventiva.auditoria or {}).get('amostra_trocas_preventivas', []) if contexto_baseline.recomputacao_sequencial_preventiva is not None else []),
        amostra_sem_cobertura=((contexto_baseline.recomputacao_sequencial_preventiva.auditoria or {}).get('amostra_sem_cobertura', []) if contexto_baseline.recomputacao_sequencial_preventiva is not None else []),
    )
