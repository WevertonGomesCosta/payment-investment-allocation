...
def main() -> None:
    ...
    render_secao_recomputacao_sequencial_central_v2(
        auditoria_central=contexto_baseline.recomputacao_sequencial_central_v2.auditoria if getattr(contexto_baseline, 'recomputacao_sequencial_central_v2', None) is not None else {},
        amostra_mudancas=((contexto_baseline.recomputacao_sequencial_central_v2.auditoria or {}).get('amostra_mudancas', []) if getattr(contexto_baseline, 'recomputacao_sequencial_central_v2', None) is not None else []),
        amostra_sem_cobertura=((contexto_baseline.recomputacao_sequencial_central_v2.auditoria or {}).get('amostra_sem_cobertura', []) if getattr(contexto_baseline, 'recomputacao_sequencial_central_v2', None) is not None else []),
    )
