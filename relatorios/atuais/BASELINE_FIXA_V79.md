...
## Ajuste incremental adicional da V79
...
A V79 não substitui a decisão local v1 vigente. Ela materializa um **benchmark shadow** que, para cada pagamento futuro/pendente, usa apenas lotes resgatáveis elegíveis e calcula uma alocação multifonte local com pesos legados de IOF, IR, idade, liquidez, cliff e VPL, apenas para comparação diagnóstica com a escolha monofonte vigente.


## Ajuste incremental adicional da V79

- criação da auditoria comparativa entre `decisao_local_v1` vigente e `resolver_hibrido_5p_shadow`;
- criação do diagnóstico `scripts/diagnostico/inspecionar_comparativo_proxy_v3_vs_hibrido_shadow.py` e wrapper correspondente;
- manutenção do benchmark híbrido como camada exclusivamente diagnóstica, sem acoplamento ao fluxo principal.


## Atualização V79

A V79 preserva a V78 e abre uma auditoria residual específica dos casos de divergência material entre o `proxy v3` vigente e o benchmark shadow do `resolver_hibrido_5p`, mantendo ambas as réguas fora do fluxo principal.
