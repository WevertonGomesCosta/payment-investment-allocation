...
- `nucleo/caixa_recebidos_auditaveis.py` → contrato mínimo da F1 + materialização de `recebido_auditavel`, `fonte_elegivel_pagamento` por pagamento, `saldo_disponivel_geral` e `decisao_local_v1` com proxy econômico v3
...
- `scripts/diagnostico/inspecionar_decisao_local_v1.py` → leitura observável da quarta estrutura real da F1 na Etapa 8
...
- materialização de `decisao_local_v1` com proxy econômico v3 por pagamento;
