...
1. A baseline atual é a **V94**.
2. A V94 preserva integralmente a baseline funcional imediatamente anterior e abre a auditoria dos casos sem cobertura integral do runner futuro shadow, com subbloco final para os 3 casos multifonte. Ela não altera o fluxo principal, o replay nem o `proxy econômico v3` congelado.
...
12. O mapa vigente de absorção da execução principal do Script 2 deve ser consultado antes de qualquer tentativa de migração do runner legado ou da competição final entre estratégias.
13. A camada `switching_economico_shadow` é diagnóstica e auditável; ela não executa switches no fluxo principal nem altera o replay/valuation vigentes.
14. A camada `resolver_hibrido_5p_shadow` é diagnóstica e auditável; ela não substitui a decisão local v1 do fluxo principal nem reabre o `proxy econômico v3` congelado.
15. O pacote final não deve incluir artefatos temporários como `__pycache__`, `.pyc`, logs brutos auxiliares, caches efêmeros não oficiais ou saídas redundantes de versões antigas.
...
94. A baseline V92 mantém uma auditoria comparativa reproduzível entre a `decisao_local_v1` vigente (proxy v3) e o `resolver_hibrido_5p_shadow`.
95. Essa auditoria usa métricas comuns e não compara diretamente scores brutos de escalas diferentes.
96. A baseline V92 também mantém uma auditoria residual específica dos casos de divergência material entre essas duas réguas.
...
99. A baseline V92 mantém uma auditoria cirúrgica específica apenas sobre os 42 casos já classificados como `potencial_reaproveitamento_proxy_v3`.
...
102. A baseline V92 mantém uma auditoria fina específica apenas da transição `Lote 3000 mar. B -> Lote 8500 mar.`.
103. A baseline V92 mantém a auditoria estrutural leve de redundância e compatibilidade focada em wrappers de compatibilidade, helpers duplicados e crescimento da superfície diagnóstica.
104. A V92 preserva a consolidação de helpers duplicados de baixo risco em três frentes: normalização de valores exauridos na apresentação, leitura simples de configuração e simulação temporal compartilhada das camadas shadow.
105. A V92 preserva a compatibilidade de `nucleo.carregador_config.obter_config` por delegação à fonte única `nucleo.config_utils.obter_config`.
106. A V92 não altera o motor; ela apenas preserva a redução de risco de divergência futura na camada estrutural leve.


## 17. Mapa vigente da execução principal do Script 2 correto

107. A baseline V92 mantém um mapa específico da execução principal do Script 2 correto, sem absorção funcional direta do runner original.
108. Esse mapa classifica o runner de switching, simulação futura, modos de execução futura e exportação final em termos de absorção imediata, posterior ou não absorção.
109. Enquanto não houver nova decisão explícita, o runner legado correto do Script 2 continua apenas como referência documental e diagnóstica.


109. A baseline V92 mantém o benchmark shadow do teste agrupado vs. individual como camada diagnóstica vinculada ao Script 1, sem substituir o fluxo principal.


## 18. Benchmark shadow do runner de simulação futura do Script 2

110. A baseline V92 mantém um benchmark shadow reproduzível do runner de simulação futura do Script 2 correto.
111. O benchmark shadow pode alterar lote principal e introduzir multifonte, mas permanece apenas diagnóstico nesta etapa.
...
114. A baseline V94 mantém uma auditoria específica dos casos sem cobertura integral do runner futuro shadow do Script 2 correto.
115. A baseline V94 mantém, como subbloco final dessa auditoria, os 3 casos multifonte do runner futuro shadow.
116. Na V94, a recomendação operacional permanece: auditar primeiro as perdas de cobertura integral e só depois os casos multifonte.
